...
const PlanGuard: React.FC<PlanGuardProps> = ({
  ...
  if (!hasAccess) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-8 bg-gradient-to-br from-slate-900 to-slate-950">
        <div className="max-w-md w-full bg-white/10 backdrop-blur-md rounded-2xl p-6 sm:p-8 border border-white/20 text-center space-y-6">
          {/* Icon */}
          ...
          <div className="space-y-3">
            <Button
              ...
              className="w-full bg-gradient-to-r from-primary to-emerald-600 hover:from-primary-dark hover:to-emerald-700 text-white font-semibold py-3 rounded-xl transition-all duration-300 transform hover:scale-105"
            >
          ...
          {userPlan && (
            <div className="mt-4 p-3 bg-white/5 rounded-lg border border-white/10">
              <p className="text-sm text-gray-400">
                Seu plano atual: <span className="text-white font-medium">{getPlanLabel(userPlan)}</span>
              </p>
            </div>
          )}
        </div>
